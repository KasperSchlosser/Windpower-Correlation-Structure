...
for zone, color, ax in zip(parameters["Zones"], plt.rcParams["axes.prop_cycle"].by_key()["color"], axes):
    ...
    ax.plot(raw_ens.index, raw_ens[zone].iloc(axis=1)[1:], color="grey", alpha=0.3)
    # highlight the HPE
...
lines = []
for zone, color, offset in zip(
    parameters["Zones"], plt.rcParams["axes.prop_cycle"].by_key()["color"], [0, -0.05, -0.1, -0.15]
):
...
for zone, color, ax in zip(parameters["Zones"], plt.rcParams["axes.prop_cycle"].by_key()["color"], axes):
    ...
    ax.fill_between(clean_ens.index, clean_ens[zone].min(axis=1), clean_ens[zone].max(axis=1), color=color, alpha=0.3)
...
ens_table.style.to_latex(
    save_path / "Tables" / "Ensembles.tex",
    position="h",
...
obs_table.style.to_latex(
    save_path / "Tables" / "Observations.tex",
    position="h",
    label="data:table:observations",
    caption=(
        "Summary table for observations. Problematic values are are values very close to 0, $0 \leq x < 0.01$",
        "Summary table of data",
    ),
    hrules=True,
)
...
split_table = pd.DataFrame(
    ...
    index=["Start", "End", "Number of data points"],
    columns=["Train", "Burn", "Test"],
)
split_table.style.to_latex(
