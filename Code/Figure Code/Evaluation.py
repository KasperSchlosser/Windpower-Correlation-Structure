...
Q = np.linspace(1e-6, 1-1e-6, num = 1000)
...
a1 = ax.fill_between(X, np.log(dist1.pdf(X) / dist2.pdf(X)),
                     ...
                     label = r"$\log\left(\frac{f(x)}{g(x)}\right)$")
a2 = ax.fill_between(X, np.log(dist2.pdf(X) / dist1.pdf(X)),
                     color = lines[1].get_color(),
                     label = r"$\log\left(\frac{g(x)}{f(x)} \right) $")
...
ax.plot(X2, kl1 + kl2, color = "black", label = r"Difference in Amplitude")
ax.legend()
ax.set_xlabel("x")
